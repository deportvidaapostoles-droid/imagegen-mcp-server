...
import { validateDallE2Params, validateDallE3Params, validateGeminiParams, validateOpenAICompatibleImageParams, validateGptImageParams, } from "./providers.js";
...
// ─── Server ─────────────────────────────────────────────────────────────────
const server = new Server({ name: "imagegen-mcp-server", version: "0.3.0" }, { capabilities: { tools: {} } });
...
async function handleOpenAIEdit(params) {
    ...
    let maskFile;
    if (params.mask) {
        maskFile = await toFile(Buffer.from((await parseImageInput(params.mask)).data, "base64"), "mask.png", { type: "image/png" });
    }
    const isGptImageModel = MODEL.startsWith("gpt-image");
    const size = params.size || (isGptImageModel ? "auto" : "1024x1024");
    const quality = params.quality || "standard";
        ...
        try {
            ...
            if (PROVIDER === "openai") {
                ...
                if (task.kind === "generate") {
                    ...
                    const { toFile } = await import("openai");
                    const imageFiles = await Promise.all(task.images.map(async (b64, i) => toFile(Buffer.from(b64, "base64"), `input_${i}.png`, { type: "image/png" })));
    ...
    if (kind === "edit") {
        ...
        for (const img of images) {
            ...
            if (img.startsWith("data:image/")) {
                ...
                if (!match)
                    return createErrorResponse("Invalid data URL format");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "generate_image") {
            const timeoutMs = Math.max(1000, args.timeout ?? DEFAULT_TIMEOUT) * 1000;
            if (PROVIDER === "openai") {
                return { content: await handleOpenAIGenerate({ prompt: args.prompt, size: args.size, quality: args.quality, n: args.n, timeout: timeoutMs }) };
            }
            if (PROVIDER === "gemini") {
                return { content: await handleGeminiGenerate({ prompt: args.prompt, aspect_ratio: args.aspect_ratio, n: args.n, timeout: timeoutMs }) };
            }
        }
        if (name === "edit_image") {
            const timeoutMs = Math.max(1000, args.timeout ?? DEFAULT_TIMEOUT) * 1000;
            if (PROVIDER === "openai") {
                return { content: await handleOpenAIEdit({ images: args.images, prompt: args.prompt, mask: args.mask, size: args.size, quality: args.quality, n: args.n, timeout: timeoutMs }) };
            }
            if (PROVIDER === "gemini") {
                return { content: await handleGeminiEdit({ images: args.images, prompt: args.prompt, aspect_ratio: args.aspect_ratio, n: args.n, timeout: timeoutMs }) };
            }
        }
