...
import { getUploadSources, isUploadConfigured, listRecentImages, stableImageUrl, storeImage, } from "./uploads.js";
...
// ─── Tool handlers ──────────────────────────────────────────────────────────
async function handleUploadImage(args, baseUrl) {
    if (!isUploadConfigured()) {
        return createErrorResponse("Uploads are not configured on this server: connect a Vercel Blob store to the project so BLOB_READ_WRITE_TOKEN is set.");
    }
...
async function handleRecentUploads(args, baseUrl, uploadPageUrl) {
    if (!isUploadConfigured()) {
        return createErrorResponse("Uploads are not configured on this server: connect a Vercel Blob store to the project so BLOB_READ_WRITE_TOKEN is set.");
    }
 ...
 */
function withStableUrl(stored, baseUrl) {
    const stable = stableImageUrl(baseUrl, stored.pathname);
    if (!stable)
        return stored;
    const { expiresAt, ...rest } = stored;
    return { ...rest, url: stable, signed_url: stored.url };
}
/**
 ...
 */
function howToUpload(uploadPageUrl, source) {
