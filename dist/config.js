...
const BOOLEAN_CONFIG_KEYS = new Set(['MCP_STDIO_LOGS', 'IMAGEGEN_ASYNC_ONLY', 'IMAGEGEN_BLOCKING_POLL']);
...
function normalizeConfigValue(key, value, warnings) {
    if (value === undefined)
        return undefined;
...
function withWarning(warnings, warning, fallback) {
    warnings.push(warning);
    return fallback;
}
function parseTimeout(value, warnings, fallback, label) {
    if (value === undefined)
        return fallback;
...
export function getServerRuntimeConfig(argv = process.argv.slice(2), env = process.env) {
    ...
    const provider = isProviderType(providerRaw)
        ? providerRaw
        : withWarning(warnings, `Invalid IMAGEGEN_PROVIDER '${providerRaw}', falling back to 'openai'`, 'openai');
...
export function getCliHelpText() {
    return [
        ...
        '',
        'Authentication (HTTP transports, environment only):',
        '  MCP_AUTH_MODE        none (default) | oauth',
        '  MCP_OAUTH_ISSUER     Identity provider issuer URL (required for oauth)',
        '  MCP_OAUTH_AUDIENCE   Resource identifier the token must target (required for oauth)',
        '  MCP_ALLOWED_EMAILS   Comma-separated list of users allowed to connect',
        '  MCP_ALLOWED_EMAIL_DOMAINS  Allow every verified address of these domains',
        '  MCP_ALLOWED_SUBJECTS Allow by identity provider user id',
        '  MCP_EMAIL_CLAIM      Claim carrying the email (default: email)',
        '  MCP_REQUIRED_SCOPES  Scopes the access token must carry',
        '',
        'CLI flags:',
