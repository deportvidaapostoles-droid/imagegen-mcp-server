...
const OPENAI_API_KEY = runtimeConfig.values.OPENAI_API_KEY;
const OPENAI_BASE_URL = runtimeConfig.values.OPENAI_BASE_URL;
const OPENAI_IMAGE_MODEL = runtimeConfig.values.OPENAI_IMAGE_MODEL || 'gpt-image-2';
const OPENAI_IMAGE_PROMPT = runtimeConfig.values.OPENAI_IMAGE_PROMPT || 'A simple black circle on a white background';
...
function assertImageDataPresent(image: { b64_json?: string; url?: string }, label: string) {
  if (!image.b64_json && !image.url) {
    throw new Error(`${label} returned no image payload`);
  }

  console.log(`${label}: ${image.b64_json ? 'received b64_json' : 'received url'}`);
}
...
async function testDirectOpenAI(): Promise<void> {
  ...
  if (!image) {
    throw new Error('Direct OpenAI test returned no image results');
  }

  assertImageDataPresent(image, `Direct ${OPENAI_IMAGE_MODEL}`);
}
...
async function testMcpOverStdio(): Promise<void> {
  ...
  env.DEFAULT_MODEL = OPENAI_IMAGE_MODEL;
